...
import { AuthModel, Campaign, BrandKit, Caption, GeneratedAsset } from '../models/auth';
...
export interface StyleProfile {
  ...
  layout: {
    ...
    elementHierarchy: 'text-over-image' | 'image-over-text' | 'side-by-side' | 'layered';
...
export class TemplateMemoryService {
   ...
   */
  static async learnFromApprovedWork(workspaceId: string, campaignId?: string): Promise<{
    templates: Template[];
    styles: StyleProfile[];
    insights: string[];
  }> {
    try {
      log.info({ workspaceId, campaignId }, 'Starting template learning from approved work');
      ...
      if (approvedCaptions.length === 0 && approvedAssets.length === 0) {
        log.info({ workspaceId }, 'No approved content found for template learning');
        return {
          templates: [],
          styles: [],
      ...
      const templates = this.extractTemplatesFromCaptions(approvedCaptions, workspaceId, campaignId);
      
      // Analyze patterns in approved assets to create style profiles
      const styles = this.extractStylesFromAssets(approvedAssets, workspaceId, campaignId);
  ...
  private static extractTemplatesFromCaptions(
    ...
    for (const [groupId, groupCaptions] of groupedCaptions.entries()) {
      ...
      const template = this.analyzeCaptionGroup(groupCaptions, workspaceId, campaignId);
   ...
   */
  private static isCaptionSimilar(caption1: Caption, caption2: Caption): boolean {
    // Compare basic characteristics
    if (!caption1.text || !caption2.text) return false;

    // Calculate similarity based on various factors
    const textSimilarity = this.calculateTextSimilarity(caption1.text, caption2.text);
    const lengthSimilarity = Math.abs(caption1.text.length - caption2.text.length) / Math.max(caption1.text.length, caption2.text.length);
   ...
   */
  private static generateInsights(approvedCaptions: Caption[], approvedAssets: GeneratedAsset[]): string[] {
    const insights: string[] = [];

    if (approvedCaptions.length > 0) {
      // Calculate average caption length
      const avgLength = approvedCaptions.reduce((sum, cap) => sum + (cap.text?.length || 0), 0) / approvedCaptions.length;
      insights.push(`Average approved caption length: ${Math.round(avgLength)} characters`);

      // Identify common themes/keywords
      const allText = approvedCaptions.map(c => c.text).join(' ').toLowerCase();
      const commonWords = this.getMostCommonWords(allText, 5);
      insights.push(`Common themes: ${commonWords.join(', ')}`);

      // Calculate average score if available
      const scores = approvedCaptions.map(c => c.qualityScore || 0).filter(score => score > 0);
      if (scores.length > 0) {
        const avgScore = scores.reduce((sum, score) => sum + score, 0) / scores.length;
        insights.push(`Average caption quality score: ${avgScore.toFixed(2)}/10`);
      }
    }
    ...
    if (approvedAssets.length > 0) {
      ...
      // Format distribution
      const formatCounts = approvedAssets.reduce((acc, asset) => {
    ...
    if (approvedCaptions.length > 0 && approvedAssets.length > 0) {
      insights.push(`Combined content analysis complete: ${approvedCaptions.length} captions, ${approvedAssets.length} assets`);
    }
  ...
  static async applyTemplate(
    ...
    try {
      ...
      log.info({ templateId, sourceText: context.sourceText.substring(0, 50) }, 'Applying template to generate content');
      ...
      if (context.targetLength || template.captionStructure.lengthPreferences.ideal) appliedRules.push('length_adaptation');
      if (template.captionStructure.wordChoicePatterns.length > 0) appliedRules.push('word_patterns');
   ...
   */
  private static applyWordChoicePatterns(text: string, patterns: string[]): string {
  ...
  static async getRecommendedTemplates(
    ...
    try {
      ...
      templates.sort((a, b) => {
        const scoreA = (a.performanceMetrics.averageScore || 0) * (a.performanceMetrics.approvalRate || 0) * (a.performanceMetrics.reuseCount + 1);
        const scoreB = (b.performanceMetrics.averageScore || 0) * (b.performanceMetrics.approvalRate || 0) * (b.performanceMetrics.reuseCount + 1);
      ...
      log.error({ error, workspaceId, campaignId }, 'Failed to get recommended templates');
      throw error;
    }
  }
  ...
  static async autoApplyBestTemplate(
    ...
    try {
      ...
      const recommendedTemplates = await this.getRecommendedTemplates(workspaceId, context.campaign?.id, 10);
        ...
        recommendedTemplates.map(async (template) => {
          try {
            ...
            return {
              ...
              templateAverageScore: template.performanceMetrics.averageScore || 0
            };
          } catch (err) {
            log.warn({ err, templateId: template.id }, 'Failed to apply template');
            return null;
          }
        })
      );
      ...
      const successfulResults = results.filter(r => r !== null) as Exclude<typeof results[number], null>[];
      ...
      const bestResult = successfulResults.reduce((best, current) => {
        ...
        const currentScore = (current.confidence || 0) * 0.6 + (current.templateAverageScore / 10) * 0.4;
        const bestScore = (best.confidence || 0) * 0.6 + (best.templateAverageScore / 10) * 0.4;
