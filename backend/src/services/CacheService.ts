...
export class CacheService {
  ...
  async get<T>(key: string): Promise<T | null> {
    ...
    if (entry) {
      ...
      if (now - entry.timestamp > entry.ttl) {
        ...
        this.stats.misses++
        MetricsService.trackCacheOperation('memory', false) // Cache miss
        return null
      }
      ...
      MetricsService.trackCacheOperation('memory', true) // Cache hit
    ...
    try {
      ...
      if (fileExists) {
        ...
        if (now - parsed.timestamp > parsed.ttl) {
          ...
          this.stats.misses++
          MetricsService.trackCacheOperation('file', false) // Cache miss
          return null
        }
        ...
        MetricsService.trackCacheOperation('file', true) // Cache hit
        return parsed.data
      }
    } catch (err) {
    ...
    MetricsService.trackCacheOperation('combined', false) // Cache miss
    return null
  }
  ...
  private async evictIfNeeded(): Promise<void> {
    ...
    if (this.config.maxSize && this.stats.memorySize > this.config.maxSize) {
      ...
      while (
        this.stats.memorySize > this.config.maxSize! &&
        this.cache.size > 0
      ) {
        const oldestKey = this.getOldestEntry()
        if (oldestKey) {
  ...
  async getCaptionVariations(
    key: string
  ): Promise<{ baseCaption: string; variants: string[] } | null> {
    return this.get<{ baseCaption: string; variants: string[] }>(
      `variations:${key}`
    )
  }

  async setCaptionVariations(
    key: string,
    variations: { baseCaption: string; variants: string[] },
    ttl?: number
  ): Promise<boolean> {
    return this.set<{ baseCaption: string; variants: string[] }>(
      `variations:${key}`,
      variations,
      ttl
    )
