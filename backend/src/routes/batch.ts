...
router.post('/generate', requireAuth, async (req, res) => {
  try {
    ...
    const result = await CaptionGenerator.startBatchGeneration(workspaceId, assetIds)
...
router.get('/workspace/:workspaceId/captions', requireAuth, async (req, res) => {
  try {
    const authenticatedReq = req as unknown as AuthenticatedRequest
    const { workspaceId } = req.params
...
router.put('/captions/:captionId', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ caption: updatedCaption })
  } catch (error) {
