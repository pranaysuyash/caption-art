...
const router = Router()
console.log('Initializing creative engine router')
...
const generateCreativesSchema = z.object({
  ...
  sourceAssets: z.array(z.string()).min(1, 'At least one source asset is required'),
...
router.use((req, res, next) => {
  console.log('CREATIVE ENGINE ROUTER REQ:', req.method, req.path)
  next()
})
...
router.post('/generate', requireAuth, async (req, res) => {
  console.log('🚀 Creative Engine: Starting creative generation')
  const startTime = Date.now()

  try {
    const authenticatedReq = req as unknown as AuthenticatedRequest
    const validatedData = generateCreativesSchema.parse(req.body)
    ...
    console.log(`📊 Creative Engine: Processing ${validatedData.sourceAssets.length} source assets`)
    console.log(`🎨 Brand Kit: ${brandKit.colors.primary} | ${brandKit.fonts.heading}`)
    console.log(`🎯 Campaign: ${validatedData.campaignId || 'General'}`)
    ...
    console.log(`✅ Creative Engine: Generated ${savedCreatives.length} creatives in ${processingTime}ms`)
    console.log(`📈 Coverage: ${enhancedResult.summary.platformsCovered.join(', ')}`)
    console.log(`🎨 Style Consistency: ${enhancedResult.summary.styleConsistency}% | Brand Alignment: ${enhancedResult.summary.brandAlignment}%`)
...
router.post('/repurpose', requireAuth, async (req, res) => {
  console.log('🔄 Creative Engine: Starting creative repurposing')
  ...
  try {
    ...
    if (!targetPlatforms || !Array.isArray(targetPlatforms)) {
      return res.status(400).json({ error: 'Target platforms array is required' })
    }
    ...
    console.log(`🔄 Repurposing creative ${creativeId} for platforms: ${targetPlatforms.join(', ')}`)
    ...
    console.log(`✅ Repurposing complete: Created ${variants.length} variants`)
    ...
    console.error('❌ Creative repurposing failed:', error)
    res.status(500).json({
...
router.get('/insights/:workspaceId', requireAuth, async (req, res) => {
  try {
    ...
    const referenceCreatives = AuthModel.getReferenceCreativesByWorkspace(workspaceId)
    ...
    const insights = {
      ...
      workspaceAnalytics: {
        ...
        averageApprovalRate: creatives.filter(c => c.approvalStatus === 'approved').length / creatives.length * 100
      },
      stylePatterns: {
        dominantPlatforms: platforms.slice(0, 3),
        preferredFormats: formats.slice(0, 2),
        layoutPreferences: layouts,
        visualStyle: referenceCreatives.length > 0 ? 'Learning from references' : 'Default brand kit'
      },
      recommendations: [
        referenceCreatives.length === 0 ? 'Upload reference creatives to improve style consistency' : null,
