...
export function AdCreativeEditor({ campaignId, brandKitId, initialData, onSave, onCancel }: AdCreativeEditorProps) {
  ...
  const [slots, setSlots] = useState<AdCreativeSlot[]>(initialData?.slots || [])
  const [generatedCreative, setGeneratedCreative] = useState<AdCreative | null>(initialData || null)
  const [loading, setLoading] = useState(false)
  const [activeTab, setActiveTab] = useState<'setup' | 'slots' | 'preview'>('setup')
  const [selectedSlot, setSelectedSlot] = useState<string | null>(null)

  const platforms = ['instagram', 'facebook', 'linkedin']
  const objectives = [
    { value: 'awareness', label: 'Awareness', description: 'Introduce your brand to new audiences' },
    { value: 'consideration', label: 'Consideration', description: 'Encourage research and comparison' },
    { value: 'conversion', label: 'Conversion', description: 'Drive purchases and sign-ups' },
    { value: 'retention', label: 'Retention', description: 'Keep customers engaged and loyal' }
  ]

  const funnelStages = [
    { value: 'top', label: 'Top of Funnel', description: 'Broad reach and brand awareness' },
    { value: 'middle', label: 'Middle of Funnel', description: 'Consideration and evaluation' },
    { value: 'bottom', label: 'Bottom of Funnel', description: 'Direct conversion and action' }
  ]

  const toneOptions = [
    'Professional', 'Friendly', 'Urgent', 'Inspirational',
    'Educational', 'Playful', 'Sophisticated', 'Casual',
    'Authoritative', 'Empathetic', 'Bold', 'Minimalist'
  ]
  ...
  const generateAdCreative = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch('/api/ad-creatives/generate', {
        method: 'POST',
        headers: {
  ...
  const removeVariation = (slotId: string, index: number) => {
    setSlots(prev => prev.map(slot =>
      slot.id === slotId
        ? { ...slot, variations: slot.variations?.filter((_, i) => i !== index) }
        : slot
    ))
  }
  ...
  const getCharacterCount = (content: string, platform: string, slotType: string) => {
    const config = getSlotConfig(slotType)
    const maxLength = config?.maxLength?.[platform as keyof typeof config.maxLength]
      ...
      <div style={{
        ...
        {[
          ...
          <button
            ...
            style={{
              padding: '1rem 1.5rem',
              borderBottom: activeTab === tab.id ? '2px solid var(--color-primary, #2563eb)' : 'none',
              backgroundColor: 'transparent',
              fontFamily: 'var(--font-heading, sans-serif)',
              fontWeight: '600',
              color: activeTab === tab.id ? 'var(--color-primary, #2563eb)' : 'var(--color-text-secondary, #6b7280)',
              cursor: 'pointer',
              display: 'flex',
              alignItems: 'center',
              gap: '0.5rem'
        ...
        <div style={{ display: 'grid', gap: '2rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              {platforms.map(platform => (
                <label key={platform} style={{
                  ...
                  borderRadius: '6px',
                  cursor: 'pointer',
                  backgroundColor: formData.platforms.includes(platform as any) ? 'var(--color-primary, #2563eb)' : 'var(--color-background, #f8fafc)',
                  color: formData.platforms.includes(platform as any) ? 'white' : 'var(--color-text, #1f2937)'
                }}>
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setFormData(prev => ({ ...prev, platforms: [...prev.platforms, platform as any] }))
                      } else {
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.5rem' }}>
              {toneOptions.map(tone => (
                <label key={tone} style={{
                  ...
                  borderRadius: '6px',
                  cursor: 'pointer',
                  backgroundColor: formData.tone.includes(tone) ? 'var(--color-primary, #2563eb)' : 'var(--color-background, #f8fafc)',
                  color: formData.tone.includes(tone) ? 'white' : 'var(--color-text, #1f2937)'
                }}>
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setFormData(prev => ({ ...prev, tone: prev.tone.filter(t => t !== tone) }))
                      }
                    }}
                    style={{ margin: 0 }}
                  />
          ...
          <div style={{ textAlign: 'center' }}>
            <button
              ...
              style={{
                padding: '1rem 2rem',
                backgroundColor: loading || !formData.keyMessage || !formData.cta
                ...
                fontSize: '1rem',
                fontWeight: '600',
                cursor: loading || !formData.keyMessage || !formData.cta ? 'not-allowed' : 'pointer',
                display: 'flex',
                alignItems: 'center',
                gap: '0.5rem',
                margin: '0 auto'
                ...
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary, #6b7280)' }}>
                  {formData.platforms.map(platform => (
                    <span key={platform} style={{ margin: '0 0.5rem' }}>
                      {platform}: {getCharacterCount(slot.content, platform, slot.type)}
                    </span>
                  ))}
                </div>
              </div>
                        ...
                        <div key={index} style={{ display: 'flex', gap: '0.5rem' }}>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              updateSlot(slot.id, { variations: newVariations })
                            }}
                            style={{
                              flex: 1,
                              padding: '0.5rem',
        ...
        <div style={{ display: 'grid', gap: '2rem' }}>
          {formData.platforms.map(platform => (
            <div key={platform} style={{
              ...
              <div style={{
                backgroundColor: platform === 'instagram' ? '#fff' : '#f0f2f5',
                border: platform === 'instagram' ? '1px solid #dbdbdb' : 'none',
                borderRadius: platform === 'instagram' ? '8px' : '0',
                padding: '1rem',
                maxWidth: '500px'
              }}>
                {slots.filter(slot => slot.type !== 'description').map(slot => (
                  <div key={slot.id} style={{ marginBottom: '0.75rem' }}>
                    ...
                    {slot.type === 'cta' && (
                      <div style={{
                        ...
                        backgroundColor: platform === 'facebook' ? '#1877f2' : 'var(--color-primary, #2563eb)',
                        color: 'white',
                        padding: '0.5rem 1rem',
                        borderRadius: '6px',
                        fontSize: '0.875rem',
