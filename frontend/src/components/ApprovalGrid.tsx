...
export function ApprovalGrid({
  ...
  const handleApprove = async (ids: string[]) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to approve captions')
    }
  }
  ...
  const handleReject = async (ids: string[]) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to reject captions')
    }
  }
  ...
  const saveEdit = async () => {
    ...
    try {
      await approvalClient.updateCaptionText(workspaceId, editingId, editingText)
    ...
    <div className="approval-grid">
      ...
      <div className="approval-grid-container">
        {filteredItems.map((item) => (
          <div
            key={item.id}
            className={`approval-item ${item.approvalStatus} ${selectedIds.includes(item.id) ? 'selected' : ''}`}
          >
            {/* Selection checkbox */}
            ...
            <div className="item-preview">
              ...
              <div className="preview-overlay">
                <span className="format-label">{item.asset?.mimeType || 'image'}</span>
              </div>
            </div>
              ...
              {editingId === item.id ? (
                <div className="inline-editor">
                  ...
                  <div className="editor-actions">
                    ...
                    <button onClick={cancelEdit} className="btn-small btn-cancel">
                      Cancel
                    </button>
                  </div>
                </div>
      ...
      {items.length > 0 && (
        <div className="approval-footer">
          <div className="progress-label">
            Approved: <strong>{approvedCount}</strong> / Target: <strong>{targetCount}</strong>
          </div>
          <div className="progress-bar-container">
            <div
              ...
              style={{
                ...
                backgroundColor: approvedCount >= targetCount ? '#10b981' : '#3b82f6',
              }}
            />
          </div>
