...
export class BackendClient {
  ...
  async generateCaption(imageUrl: string): Promise<CaptionResponse> {
    const response = await apiFetch(`${this.baseUrl}/api/caption`, {
      method: 'POST',
  ...
  async generateMask(imageUrl: string, model?: string): Promise<MaskResponse> {
    const response = await apiFetch(`${this.baseUrl}/api/mask`, {
      method: 'POST',
  ...
  async getMaskingModels(): Promise<MaskingModelsResponse> {
    const response = await apiFetch(
      `${this.baseUrl}/api/brand-kits/masking-models`
    );
  ...
  async verifyLicense(licenseKey: string): Promise<VerifyResponse> {
    const response = await apiFetch(`${this.baseUrl}/api/verify`, {
      method: 'POST',
  ...
  async checkHealth(): Promise<HealthResponse> {
    const response = await apiFetch(`${this.baseUrl}/api/health`);
